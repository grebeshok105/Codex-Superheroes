...
/**
 * The only writer of {@link CoreAttachments#HERO_DATA}.
 *
...
public final class HeroDataStore {
	...
	public static HeroData get(Player player) {
		return player.getAttachedOrCreate(CoreAttachments.HERO_DATA);
	}
	...
	public static HeroData update(ServerPlayer player, UnaryOperator<HeroData> change) {
		...
		player.setAttached(CoreAttachments.HERO_DATA, after);
		syncPublicHero(player, after.heroId());
		if (sameShape(before, after)) {
			player.setAttached(CoreAttachments.HERO_DATA_RESOURCES_DIRTY, Boolean.TRUE);
		} else {
	...
	/**
	 * Back-fills {@link CoreAttachments#PUBLIC_HERO} from {@link CoreAttachments#HERO_DATA} —
	...
	private static void syncPublicHero(ServerPlayer player, ResourceLocation heroId) {
		if (heroId == null) {
			player.removeAttached(CoreAttachments.PUBLIC_HERO);
		} else {
			player.setAttached(CoreAttachments.PUBLIC_HERO, heroId);
		}
	}

	private static void sendFull(ServerPlayer player, HeroData data) {
		player.removeAttached(CoreAttachments.HERO_DATA_RESOURCES_DIRTY);
		CoreNetworking.syncHeroData(player, data);
	}

	private static void flushResources(ServerPlayer player) {
		if (player.removeAttached(CoreAttachments.HERO_DATA_RESOURCES_DIRTY) != null) {
			CoreNetworking.syncResources(player, get(player));
		}
	}
