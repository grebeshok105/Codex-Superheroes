 ...
 * {@link CoreAttachments#ABILITY_COOLDOWNS} so a hero swap or relog cannot reset them
...
public final class AbilityCooldowns {
	...
	public static void setCooldownTicks(ServerPlayer player, ResourceLocation abilityId, int ticks) {
		...
		player.setAttached(CoreAttachments.ABILITY_COOLDOWNS, Map.copyOf(cooldowns));
	...
	public static void clearAndSync(ServerPlayer player) {
		Map<ResourceLocation, Long> cooldowns = player.getAttached(CoreAttachments.ABILITY_COOLDOWNS);
		if (cooldowns == null || cooldowns.isEmpty()) {
			return;
		}
		player.setAttached(CoreAttachments.ABILITY_COOLDOWNS, null);
