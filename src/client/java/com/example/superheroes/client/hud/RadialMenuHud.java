...
public final class RadialMenuHud {
	...
	/** Совместимость с новым клиентским циклом — оригинальной радиалке тики анимации не нужны. */
	...
	private static void drawHub(GuiGraphics graphics, int cx, int cy, HeroTheme theme) {
		if (com.example.superheroes.client.render.WildShaders.rectReady()) {
			// GLSL: стеклянный круглый хаб с тонким кольцом и мягким неоном темы
			int glow = (0x55 << 24) | (theme.radialGlow() & 0x00FFFFFF);
			com.example.superheroes.client.render.WildRenderer.orb(graphics, cx, cy, 23f,
					0xCC080A14, theme.radialBorderIdle(), 1.1f, glow, 14f);
			return;
		}
	...
	private static void drawBackplate(GuiGraphics graphics, int cx, int cy) {
		if (com.example.superheroes.client.render.WildShaders.rectReady()) {
			// GLSL: мягкая круглая виньетка вместо квадратного градиента
			com.example.superheroes.client.render.WildRenderer.orb(graphics, cx, cy,
					BACKPLATE_RADIUS, 0x66060814, 0, 0f, 0x4A060814, 52f);
			return;
		}
	...
	private static void drawCursor(GuiGraphics graphics, Minecraft mc, int cx, int cy, HeroTheme theme) {
		...
		if (com.example.superheroes.client.render.WildShaders.rectReady()) {
			int glow = (0x99 << 24) | (theme.radialGlow() & 0x00FFFFFF);
			com.example.superheroes.client.render.WildRenderer.orb(graphics, px, py, 3.4f,
					theme.radialTextActive(), theme.radialBorderActive(), 1.0f, glow, 9f);
		} else {
	...
	private static void drawSlot(GuiGraphics graphics, int x, int y, int width, int height,
		...
		if (com.example.superheroes.client.render.WildShaders.rectReady()) {
			// GLSL: те же слоты, но со сглаженными углами и мягким неоном у выбранного
			int glow = selectedSlot ? ((0x88 << 24) | (theme.radialGlow() & 0x00FFFFFF)) : 0x30000000;
			com.example.superheroes.client.render.WildRenderer.panel(graphics, x, y, width, height,
					5.5f, top, bottom, border, selectedSlot ? 1.2f : 0.9f, glow, selectedSlot ? 16f : 6f);
