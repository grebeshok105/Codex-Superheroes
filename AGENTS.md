...
Это Fabric-мод `superheroes` для Minecraft `1.21.1` на Java 21. Версия `4.0.0`, package `com.example.superheroes`, official Mojang mappings, Fabric Loader `0.19.2`, Fabric API `0.116.12+1.21.1`, GeckoLib `4.5.8`. Render-слой: Veil `4.1.2` (runtime) и Iris (compile-only bridge для Mirror Dimension ability).
