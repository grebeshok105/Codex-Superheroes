...
| Ресурс | Energy max 200, реген 1.0/тик. (Хотели ману как «магу», но в движке мода авто-реген есть только у energy — Regulus гоняет ману отдельным контроллером; для одной способности городить контроллер регена не стоит.) |
| Стоимость | 60 энергии активация + 0.5/тик пока активна (≈10/сек) |
...
| Длительность | пока toggle активен и хватает маны; деактивация — повторное нажатие/смерть/растрансформация/энергия в 0 |
...
| Cleanup | деактивация при: смерти кастера, смерти цели, дисконнекте любого, растрансформации кастера, нехватке энергии; на клиенте — deadman-switch и restore-файл (§4.4) |
...
`OK_APPLIED`, `NO_IRIS`, `NO_PACK`, `IRIS_API_FAIL`, `OK_RESTORED`. Контроллер мапит на локализованные сообщения кастеру. При `NO_IRIS`/`NO_PACK`/`IRIS_API_FAIL` контроллер **сам останавливает** эффект (не жжём ресурс впустую).
...
- `DoctorStrangeHero`: energy-герой (energy 200, реген 1.0/т; мана в движке не регенится без отдельного контроллера), стандартные dimensions, `getAbilities() = [MIRROR_DIMENSION]`, `getDefaultBinding -> ENERGY`.
...
- [ ] Энергия тикает вниз, при 0 — авто-выключение + кулдаун 10с.
