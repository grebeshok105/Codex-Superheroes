...
- Внешние зависимости: П5 H — go (без него волны не начинаются); BF5 и BF6 — до I4; BF6, BF8, BF9 — до I5. Используются registrar'ы и сервисы П3–П5: `HeroModuleContext` (+`content/payloads/attachments`), `HeroClientContext`, `OwnedSessionMap`, `Motion`, `FxBroadcast`, `Targeting`, `SkinResolver`, `HudLayers`, `AbilityDecorations`, `ClientSoundFilters`.
