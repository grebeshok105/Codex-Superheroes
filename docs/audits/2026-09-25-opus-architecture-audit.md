...
| B18 | ✅ исправлено | Армия Sung перенесена из статических `Map`/`Set` в persistent-attachment `SUNG_SHADOW_ARMY` (UUID теней + флаги `summoned`/`phase2`): после рестарта сервера сохранённые тени перелинковываются по UUID, дубликат армии не спавнится. Тень-сирота (владелец офлайн/снял героя/её нет в армии) сбрасывает цель и не атакует; онлайн — удаляет себя, как при `disbandAll` для невыгруженных теней. Этап 13. |
| B19 | ✅ исправлено | `combat/TargetFilters` — единый предикат враждебного выбора целей (`harmableBy`/`hostileTo`), заменивший ~25 приватных `isValidTarget` и ~60 инлайн-лямбд; PvP и команды учитываются через ванильный `victim.canHarmPlayer(attacker)` (covers `pvp=false` и friendly-fire команд). `magic()` без атакующего заменён на `indirectMagic` в ScaramoucheWindPrison/ThanosSoulPulse/MonarchsDomain/UraniumDagger. Этап 14. |
| B22 | ✅ исправлено | Новый `util/SafeTeleport.clamp(level, entity, dest)` идёт по отрезку до цели и возвращает последнюю свободную позицию (невыгруженный чанк = препятствие). Переведены: Goku IT (включая `look*12`), Loki Tesseract, Scorpion Fire Teleport, Kratos God Slayer, Reinhard Speed Judgment, Kawarimi, Shadow Exchange (оба конца), Doom Grip lunge, Thanos Space Portal. Намеренно не тронуты: телепорты удержания/якоря и возврата (DoomGrip hold, Spatial Bind, Greed Cage, Unibeam, IronMan anchor, Pandora/Mirror Dimension, lockPos у Reinhard/Raiden/Heavens Strike) — позиции там записаны заранее и по дизайну непробиваемы, и скриптовые телепорты кат-сцены Regulus counter. Этап 13. |
| Потенциальные (4 шт.) | ✅ исправлено | Все 4 подтверждены по коду. Орда: `HordeManager.tickInstance` останавливается, если ни один живой не-спектатор игрок не ближе 96 блоков к центру (таймеры, отложенные спавны и reconcile заморожены; `removeWhenFarAway=false` оставлено нарочно — мобы «безжалостные»). Рам: контроллер хранит UUID вместо живой ссылки, `findRam` подхватывает живую незарегистрированную Рам и дискардит дубли, `RamEntity.shouldBeSaved()→false` (сессионный призыв не сохраняется в NBT). Mixin: `@Shadow`-поле заменено на `falbiks/HeroComponentAccess` — ленивый fail-soft доступ через reflection с обходом иерархии классов и однократным warn. Конфиг клиента: `load` ловит `IOException|RuntimeException` (Gson кидает `JsonParseException` — RuntimeException), дефолты + warn; `save` пишет в `.tmp` и `Files.move` с `ATOMIC_MOVE` и фолбэком. Регрессии — `Stage15GameTests` (орда на паузе без зрителей и возобновляется; Рам подхватывается и дедуплицируется). Этап 15. |
| Мелочи | ✅ исправлено | `fabric.mod.json`: `minecraft ~1.21.1`, `fabric-api >=0.116.12`; модель `horde_crystal` (текстура `minecraft:item/echo_shard`); 5 недостающих entity-имён в en+ru (`horde_acid_bomb`, `horde_fire_bomb`, `kage_bunshin`, `shield_projectile`, `smart_missile`); 21 кириллический `Component.literal` → translatable-ключи (+22 ключа в обеих lang); `AttachmentRegistry.builder()` → `AttachmentRegistry.create()` (13 attachment'ов). Гарды: `assertNoCyrillicLiterals`, `assertEntityLangNames`. Этап 12. |
| §3 «Сеть» | ✅ исправлено | Ресурсы: коалесцированный flush этапа 2 уже слал один пакет на игрока в конце тика — добавлен троттл до 1 пакета / 10 тиков (гранулярность HUD 0.5с). Армия Sung: рассылка только владельцу + `PlayerLookup.tracking(owner)` и только при изменении состояния (плюс пересылка раз в 100 тиков для вошедших в радиус). Unibeam: заряд — 1 пакет вместо ~26; луч — count-батчи (~60 пакетов вместо ~400), FLAME/LAVA — по 1 батчу на весь луч. Kamehameha: ~3×60→~3×20 пакетов (count=3 на сегмент с вытянутым спред-боксом). Эффекты: новый `effect/EffectRefresh` — переприменение только при отсутствии или <50% остатка; применён в Unibeam (anchor+debuffs), IronManReactor, RegulusMadness, RemDemonism crater, Scorpion breath. Этап 15. |
| §3 «Релизы» | ⚠️ частично | `release.yml` в репо нет (находка устарела); `auto-approve-pr.yml` — решение владельца репозитория, не код. |
...
| Долг 3 | ✅ | `HeroTickDispatcher` (этап 11): фазы GLOBAL→LEVELS→PLAYERS→ABILITY_ACTIVE, единый `END_SERVER_TICK`, мёртвые игроки скипаются один раз централизованно (B17 закреплено GameTest'ами), `HeroData` читается один раз на игрока; bootstrap-монолит `SuperheroesMod` разбит на `registerTickHandlers()`/`registerPlayerLifecycle()` — порядок виден в таблице. Этап 11b: все ~50 контроллеров мигрированы на `HeroTickDispatcher` через `registerTickHandlers()` (GLOBAL: 25 регистраций, PLAYERS: 33); сознательно оставлены саморегистрирующимися: `HeroDataStore` (фаза `hero_data_flush` — граница фаз by design), `MirrorDimensionController` (этап 8), `MadnessFlightController`/`KratosRageController`/`ThanosGauntletStateController` (их `START_SERVER_TICK`-регистрации — диспетчер работает только на END). |
...
| Долг 6 | ✅ закрыт | Transient-модификаторы и замки (этап 3) закрыли утечки баффов при смене/снятии героя; `lifecycle/PassiveReconciler` (этап 9) — объявленные пассивки переживают молоко/смерть и снимаются при потере героя, `LivingEntityFallDamageMixin` ограничил fall-иммунитет участниками контра. Этапы 3 и 9. |
...
| 13    | Тени Sung переживают рестарт/снятие героя; телепорты проверяют коллизии                                          | B18, B22      | M / низкий         |
| 14    | Общий предикат выбора целей с учётом PvP и команд                                                                | B19           | M / средний        |
| 15    | Потенциальные находки + сетевые улучшения §3                                                                      | опционально   | M / низкий         |
