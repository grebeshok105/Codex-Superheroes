...
| §3 «Релизы» | ⚠️ частично | `release.yml` в репо нет (находка устарела); `auto-approve-pr.yml` — решение владельца репозитория, не код. || Долг 2 | ✅ закрыт | Все ~20 прямых писателей переведены на `HeroDataStore`; `ProjectSanityTest.assertHeroDataHasSingleWriter` запрещает прямой `setAttached(HERO_DATA)`/sync вне стора. Полный sync — сразу (порядок с другими пакетами), энергия/мана — один пакет в конце тика (фаза `hero_data_flush`). Этап 2. |
...
| Долг 7, 8, 9 | ⏳ | Тонкий контракт `Ability`, дубли механик, клиент-монолит — вне этапов 1–11, отдельный план. |Новые находки, обнаруженные при реализации:
